...
def solve(res,mode):
    ...
    soc={k:0.5*D[k]['cap'] for k in D}; tot=0.0; pos=0; peak=0.0
    for _ in range(days):
        sl=slice(pos,pos+H)
        ...
        if mode=='indiv':
            ...
            for t in range(H):
                peak=max(peak,sum(v['_gi'][t].value() for v in D.values()))
        else:
            tot+=sum((GI[t].value()*ip[sl][t]-GE[t].value()*ep[sl][t])*dt/1000 for t in range(H))
            for t in range(H): peak=max(peak,GI[t].value())
...
for r in ['h','q']:
    for mode in ['indiv','coord']:
        c,pk=solve(r,mode); res[f'{r}_{mode}']=c; res[f'{r}_{mode}_peak']=pk
        print(f'{r} {mode}: {c:.0f} EUR  peak {pk:.0f} kW  ({time.time()-t0:.0f}s)')
